...
public class Board {
    ...
    public Board playMark(int oneIndexedPosition, Mark mark) {
        if (validPosition(oneIndexedPosition)) {
            cells.set(zeroIndexed(oneIndexedPosition), mark);
        }
    ...
    public boolean validPosition(int oneIndexedPosition) {
        return positionIsWithinRange(oneIndexedPosition) && !cellIsOccupied(zeroIndexed(oneIndexedPosition));
    }
    ...
    public List<Integer> findPositions(Mark mark) {
        return range(0, cells.size())
                .filter(p -> cells.get(p) == mark)
                .mapToObj(i -> i)
                .collect(toList());
    }

    public Mark findMarkAtDisplayPosition(int oneIndexedPosition) {
        return findMarkAtIndex(zeroIndexed(oneIndexedPosition));
    }
    ...
    private int zeroIndexed(int oneIndexedPosition) {
        return oneIndexedPosition - DISPLAY_POSITION_OFFSET;
    }
